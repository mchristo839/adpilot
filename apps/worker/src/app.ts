...
import { BriefSchema, regenerateCreative, runBrief } from "./services/brief.js";
...
import { dailySummaries, dailySummary, sendDailySummaries } from "./services/report.js";
...
app.get("/health", (c) => {
  const ctx = getCtx();
  return c.json({ ok: true, dry_run: ctx.dryRun, writes_allowed: ctx.meta.scopes?.writes_allowed ?? null, version: env.META_API_VERSION });
});
...
app.post("/brands", async (c) => {
  ...
  await audit(ctx.db, { actor: env.APPROVER_NAME, entity_type: "brand", entity_id: inserted.id, action: "create", after_json: inserted });
...
app.patch("/brands/:id", async (c) => {
  ...
  await audit(ctx.db, { actor: env.APPROVER_NAME, entity_type: "brand", entity_id: before.id, action: capChanged ? "caps.update" : "update", before_json: before, after_json: after });
  return c.json(after);
});
...
app.post("/brief", async (c) => {
  ...
  const campaign = await runBrief(ctx, brief);
...
app.get("/campaigns/:id", async (c) => {
  ...
  const creatives = (must(await ctx.db.from("creatives").select("*").eq("campaign_id", id).order("created_at"), "creatives") as Creative[]).map((cr) => ({
    ...cr,
    image_url: cr.image_path ? `${env.STORAGE_PUBLIC_URL}/${cr.image_path}` : null,
    image_url_story: cr.image_path_story ? `${env.STORAGE_PUBLIC_URL}/${cr.image_path_story}` : null,
  }));
...
app.post("/campaigns/:id/creatives/:cid/regenerate", async (c) => {
  ...
  const cr = await regenerateCreative(ctx, c.req.param("id"), c.req.param("cid"), part);
  return c.json(cr);
});
...
app.post("/campaigns/:id/approve", async (c) => {
  ...
  const campaign = await approveAndPublish(ctx, c.req.param("id"), creative_ids, env.APPROVER_NAME);
  return c.json(campaign);
});

app.post("/campaigns/:id/discard", async (c) => {
  const ctx = getCtx();
  await discardCampaign(ctx, c.req.param("id"), env.APPROVER_NAME);
...
app.post("/kill", async (c) => {
  ...
  const result = await killSwitch(ctx, brand, env.APPROVER_NAME);
